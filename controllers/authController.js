...
export const forgotPasswordController = async (req, res) => {
    try {
        ...
        if (!answer) {
            res.status(400).send({
                message: 'answer Answer Must Required',
            })
        }
        if (!newPassword) {
            res.status(400).send({
                message: 'New Password Must Required',
            })
        }

        //check email and answer are true then change password
        ...
        if (!user) {
            res.status(400).send({
                success:false,
                message:'Wrong Email or answer Answer',
            })
        }
